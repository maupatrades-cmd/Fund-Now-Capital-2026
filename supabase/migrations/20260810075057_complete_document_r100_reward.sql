...
declare
 ...
 v_lock public.complete_document_reward_locks%rowtype;
 v_date date := (now() at time zone 'Africa/Johannesburg')::date;
 v_cycle date;
 v_docs integer;
begin
 if not public.is_owner() then
  raise exception 'Only the owner can lock the R100 reward' using errcode='42501';
 end if;
 if p_beneficiary_role not in ('partner','contractor','lead_referrer') then
  raise exception 'Unsupported beneficiary role';
 end if;
 perform pg_advisory_xact_lock(hashtext('complete_document_reward:'||p_deal_id::text));
 select * into v_lock from public.complete_document_reward_locks where deal_id=p_deal_id;
 if found then return v_lock; end if;
 select * into v_deal from public.deals where id=p_deal_id;
 if not found then raise exception 'Deal not found'; end if;
 if v_deal.stage<'submitted'::public.deal_stage or v_deal.stage='declined'::public.deal_stage then
  raise exception 'Deal must be submitted to a funder first';
 end if;
 if not exists(select 1 from public.deal_package_dispatches where deal_id=p_deal_id) then
  raise exception 'No recorded funder package dispatch';
 end if;
 ...
 if exists(select 1 from public.documents where deal_id=p_deal_id and is_current_version and status='active' and verification_status<>'accepted') then
  raise exception 'Every current active deal document must be accepted';
 end if;
 if v_deal.is_purchase_order and not exists(select 1 from public.documents where deal_id=p_deal_id and is_current_version and status='active' and verification_status='accepted' and document_type='purchase_order') then
  raise exception 'Accepted purchase order required';
 end if;
 select * into v_lead from public.leads where id=v_deal.lead_id;
 if not found then raise exception 'Deal has no source lead'; end if;
 select * into v_profile from public.profiles where id=p_beneficiary_profile_id and is_active;
 if not found then raise exception 'Active beneficiary not found'; end if;
 if p_beneficiary_role='contractor' and (v_profile.role::text<>'contractor' or v_lead.attributed_to_contractor_id is distinct from v_profile.id) then
  raise exception 'Contractor attribution mismatch';
 end if;
 if p_beneficiary_role='partner' and (v_profile.role::text<>'partner' or v_lead.attributed_to_partner_id is distinct from v_profile.id) then
  raise exception 'Partner attribution mismatch';
 end if;
 if p_beneficiary_role='lead_referrer' and (v_profile.role::text<>'lead_referrer' or v_lead.attributed_to_lead_referrer_id is distinct from v_profile.id) then
  raise exception 'Lead-referrer attribution mismatch';
 end if;
 if extract(day from v_date) <= 22 then
  v_cycle := date_trunc('month',v_date)::date;
 else
  v_cycle := (date_trunc('month',v_date)+interval '1 month')::date;
 end if;
 ...
 select p.id,p.email,p.role::text,'CREATE','complete_document_reward_lock',v_lock.id,'Exactly R100 complete-document reward locked',to_jsonb(array_remove(array[v_deal.id,v_deal.client_id],null)) from public.profiles p where p.id=(select auth.uid());
 return v_lock;
end;
$$;
