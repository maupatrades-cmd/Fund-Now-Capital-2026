...
create policy owner_tasks_owner_all on public.owner_tasks for all to authenticated
...
do $$ begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='owner_tasks' and policyname='owner_tasks_owner_all') then raise exception 'assert: owner task policy missing'; end if;
