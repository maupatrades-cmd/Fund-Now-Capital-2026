...
- **FIX-A — RPC grants hardening — 🔨 THIS PR** (`claude/fix-a-rpc-grants-hardening`, migration `20260731233000_fix_a_rpc_grants_hardening.sql`). Owner precondition MET: src/ searched — ZERO anon-dependent RPC callers (single anon-key client, all 15 `.rpc()` sites behind OwnerGate ⇒ authenticated JWT; pre-auth pages call only `supabase.auth.*`). Migration: revoke anon+PUBLIC on the 10-fn API surface (authenticated re-granted explicitly), strip ALL API-role EXECUTE from 29 trigger/internal fns, `supabase_auth_admin` grant kept on `handle_new_user`, `leads_qualification_guard` search_path pinned (body verified pin-safe), 4 self-check assertions. **Dry-run executed against live in a rolled-back txn: PASS.** Apply to live after merge per schema workflow.
