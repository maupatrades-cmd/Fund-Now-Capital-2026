...
- **Post-merge steps:** 1) apply migration to live; 2) owner assigns the contractor role to the existing test user `maupatrades@gmail.com` (auth user + profiles row already live, currently role `partner` via the handle_new_user default) in the Dashboard SQL editor — `update public.profiles set role = 'contractor' where id = (select id from auth.users where email = 'maupatrades@gmail.com') returning id, email, role;` — expect exactly 1 row back (profile id `ce628fd1-5280-450a-8c14-71b83e2d4af0`); sign that account out/in before testing (role is cached a few minutes); 3) acceptance: owner login → /dashboard · Doctor → /partner · contractor (maupatrades@gmail.com) → /contractor welcome shell · contractor hits /dashboard → bounced to /contractor · owner hits /contractor → bounced to /dashboard.
