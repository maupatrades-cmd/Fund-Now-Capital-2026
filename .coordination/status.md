...
- **Scope shipped (3 migrations, verified via non-persisting BEGIN/ROLLBACK dry-run against live — DDL applies clean, all 14 function bodies compile, calc math + columns/signatures pass; NOT yet applied):**
  ...
  - `20260803120100_picker_backend.sql` — (1) picker columns on `deals`: `commission_calculation jsonb` `{base,rate,tier_modifiers[],timing,adjustments[]}`, `commission_state text` (CHECK potential/pending/locked; NULL = no picker), `estimated_gross_commission`/`actual_gross_commission numeric(14,2)`, `commission_locked_at timestamptz`, `commission_locked_by uuid` (FK→profiles, NOT VALID here). (2) 7 calc-base SECURITY DEFINER helpers (`calc_percent_of_gross_funded`/`_finance_charge`/`_revenue_collected`/`_mdr`/`_client_interest`, `calc_points_margin`, `calc_flat_rand_per_deal`) — rate is a whole-number percent (10 = 10%), all `numeric(14,2)`, authenticated-only. (3) 5 state RPCs `set_commission_picker` / `update_commission_picker` / `transition_to_pending` / `transition_to_locked` / `unlock_commission` — belt-and-braces DEFINER + `search_path=''` + `is_owner()` + advisory xact lock + state guard + `activity_logs` write + idempotent same-state no-op; grants authenticated+postgres+service_role, anon/PUBLIC revoked. (4) 2 read RPCs `get_deal_commission_state` / `list_deals_by_commission_state` (owner-only). (5) internal helpers `log_commission_picker_activity` + `validate_commission_calculation` (no API grant). Structural + behavioural DO-block assertions (behavioural runs at real apply — it inserts the new enum values, which can't be used in the same tx they're added).
  - `20260803120200_picker_backend_locked_by_index.sql` — statement-only, OUTSIDE tx: `CREATE INDEX CONCURRENTLY idx_deals_commission_locked_by` + `VALIDATE CONSTRAINT deals_commission_locked_by_fkey` (CLAUDE.md FK-on-populated-table discipline).
- **Explicitly OUT of scope (later PRs, per brief):** UI; partner "Potential Earning" display; C2 money-engine hand-off (`write_commission_record`) at LOCK; Sprint-1b tier engine; notification triggers on state transitions. This PR only captures picker state + stores owner-supplied estimated/actual gross.
- **Privacy (POPIA + commission privacy):** every picker column + RPC is owner-only (`deals` is owner-ALL under RLS; every RPC also gates on `is_owner()`). No partner surface added.
- **Files touched:** the 3 migrations above + this status.md section only. No frontend, no other lane's files. Zero collision with ROADMAP-UPDATE (PR #105, ROADMAP.md only) or any other active lane.
...
- **POST-MERGE steps (owner / applier):** apply the 3 migrations to live IN ORDER (`enums` → `picker` → `locked_by_index`); the 2nd runs the behavioural assertions (rolls back on any failure); the 3rd must run OUTSIDE a transaction (CONCURRENTLY) — verify `pg_index.indisvalid = true` after. Then the acceptance test (set → get → →pending → →locked → set-on-locked raises → unlock → non-owner denied).
