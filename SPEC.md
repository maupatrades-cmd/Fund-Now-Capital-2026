...
**Duplicate detection (B2.3 scope, not built in B2.2):** at lead-entry time, surface an **advisory warning** on likely duplicates by matching `business_name`, `cipc_number`, and `contact_email` similarity against existing leads/clients — nudging the owner not to enter the same business twice. Note this is a soft warning, not a hard block: it does not by itself prevent a duplicate deal, and two separate lead rows for the same business can each still be qualified into their own deal. Hard prevention of a second deal for a client that already has one is a separate server-side guard (a client/deal-level uniqueness check, or a qualification-time "already has an active deal — override?" gate) — scoped and decided in B2.3, not implied by the entry-time warning. Until any of this lands, accidental duplicates are cleaned up owner-side via SQL (see ROADMAP "Deferred Polish").
