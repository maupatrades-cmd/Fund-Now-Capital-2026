...
**Duplicate detection (B2.3 scope, not built in B2.2):** at lead-entry time, warn on likely duplicates by matching `business_name`, `cipc_number`, and `contact_email` similarity against existing leads/clients — so the same business isn't entered (and later qualified) twice. This is the guard that would prevent a duplicate deal being created for a client that already has one; until it lands, accidental duplicates are cleaned up owner-side via SQL (see ROADMAP "Deferred Polish").
