...
**`deals` table — schema additions (B2.2, live):** the Phase A `deals` table (id, reference `DEAL-xxx`, client_id, referral_partner_id, stage `deal_stage`, is_purchase_order, amount_requested, gross_commission, awarded_funder_id, declined_reason, stage_entered_at, is_priority, notes, created_by, created_at, updated_at) gains three columns: **`lead_id`** uuid FK → `leads` (nullable; the originating-lead link, with a partial UNIQUE index enforcing one deal per lead), **`funding_purpose`** jsonb (default `[]`; copied from the lead), and **`funding_timeline`** `lead_funding_timeline` enum (copied from the lead). Existing deals have `lead_id` null and are exempt from the unique index.
