...
When `referred_by = 'other'`, `referred_by_other` captures the free-text referrer name. When `referred_by ∈ ('self','bright_destiny')`, `referred_by_other` should be null. (B2.1: `entered_by` is pinned to the caller by a BEFORE INSERT trigger — not client-supplied.)
