...
**Contracted-funder gate (C0.3, LOCKED — Option A):** A further distinction *within* the 21 CRM funders: `funders.is_contracted` (bool, default false) marks the subset FNC holds a **signed** agreement with. Only contracted funders can carry `funder_commission_structures` rows — enforced at the DB layer by the `enforce_rate_funder_contracted` BEFORE-trigger (a Postgres CHECK can't reference another table, so the cross-table gate is a trigger, not a CHECK) — and only contracted funders are invoiceable (C1). `funders.short_code` (owner-set, e.g. `POLLEN`) feeds the invoice payment reference (`POLLEN-INV0032-CHICKANOS`). Rate structures also gain `payment_terms_days` (0 = "Due on receipt") + `contract_reference` ("Lead Provider Agreement dated 28 April 2026"); `deal_funder_submissions` gains `finance_charge_amount` + the new `funder_rate_type` value `percent_of_finance_charge` (Pollen-style: commission = % of the client's finance charge, not the funded amount). **No backfill** — the owner flags the ~7 real contracted funders and enters each `short_code` by hand. This is an internal *invoiceability* flag, **not** a new public funder figure: the LOCKED 43+/21 split above is unchanged. *An earlier proposal to split invoiceable funders into a separate `contracted_funders` table was rejected — no such table ever existed (verified against live schema 2026-07-20); the flag on `funders` supersedes that idea, so there is nothing to deprecate.*
