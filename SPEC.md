...
Frontend note: the client Documents tab component + hooks were **parameterized by owning entity** (`DocOwner = { type: 'client' | 'lead', id }`) — the same `DocumentsPanel` now serves both the client tab and a new lead-detail Documents section, and the upload path follows `{entity_type}s/{id}/…` for leads too.
