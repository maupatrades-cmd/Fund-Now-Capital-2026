...
PHASE B IN PROGRESS: B1 (Industry Classification) done, live. B2.1 (Lead Entry) / B2.2 (Qualification Workflow — `qualify_lead`) / B2.3 (Duplicate Detection + lead activity logging + lead-lifecycle notifications & email) done, applied + verified live; B2.3 smoke test passed and test-data cleanup complete. B3 (Document Management upgrade) next.
REAL LIVE DATA (only production rows in the DB — all test leads/deals/clients removed 2026-07-17): Mama Mabase JV (DEAL-001, funded) · Fepa Sechaba Milling and Mining (DEAL-002) · NRL BAKWENA MINE (DEAL-004).
NEXT: B3 Document Management upgrade (SPEC S6 core slice) · A13 paperwork (external, in progress).
