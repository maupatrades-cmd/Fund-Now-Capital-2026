...
Before deploying an Edge Function, set every environment variable it reads as a Supabase Edge Function secret. Functions that verify an inbound webhook (e.g. the A12 `send-notification-email` flow) need `WEBHOOK_SECRET`, and any secret shared with the DB must match its Vault counterpart exactly — otherwise the function 401s every call. When a Claude Code session's proxy blocks outbound HTTP, use `pg_net` from within the database for readiness probes — it's the same path production uses.
