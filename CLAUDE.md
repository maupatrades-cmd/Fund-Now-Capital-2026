...
NEXT: B4 (Client Story + Call Logs — first real capture: Fepa Sechaba / Tumi) · B5 (SA Data Validation — ID Luhn, CIPC format, cell, duplicate detection) · then Phase C opens with **C0** (per-funder commission structures — prerequisite for C1/C2; scope proposal first, see ROADMAP C0 + SPEC S7A) · A13 paperwork (external, in progress).
