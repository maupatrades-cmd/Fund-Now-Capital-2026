...
// Reports v1 data layer — owner-only, read-only. Four small queries feed the
// whole dashboard; each is guarded by the existing owner-all RLS on its table
// (commission_records / leads / referral_partners / profiles). All embeds are
// !left per the nullable-FK rule so a legacy row never vanishes.
//
...
export function useReports() {
  ...
  const isLoading =
    commissions.isLoading || leads.isLoading || partners.isLoading || contractors.isLoading;
  const isError =
    commissions.isError || leads.isError || partners.isError || contractors.isError;
  ...
  const data = useMemo(
    ...
    [commissions.data, leads.data, partners.data, contractors.data],
  );
