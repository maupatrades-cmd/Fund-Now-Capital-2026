...
export default function DealPackagePage() {
  ...
  const documents = (docsQuery.data ?? []).filter((doc) => doc.is_current_version && doc.status === "active");
  return <div className="mx-auto max-w-4xl space-y-6 bg-white p-6 print:max-w-none print:p-0">
    ...
    <section><h2 className="text-lg font-semibold text-brand-navy">Included document register</h2><p className="mt-1 text-sm text-muted-foreground">The files remain in the secure document vault; this register identifies the current versions for submission.</p><table className="mt-4 w-full text-left text-sm"><thead><tr className="border-b"><th className="py-2">Document</th><th>File</th><th>Verification</th><th>Version</th></tr></thead><tbody>{documents.map((doc)=><tr key={doc.id} className="border-b"><td className="py-3">{docTypeLabel(doc.document_type)}</td><td>{doc.filename}</td><td className="capitalize">{doc.verification_status}</td><td>v{doc.version_number}</td></tr>)}</tbody></table>{documents.length===0&&<p className="mt-4 rounded-lg bg-amber-50 p-4 text-sm text-amber-800">No current documents are attached to this client.</p>}</section>
