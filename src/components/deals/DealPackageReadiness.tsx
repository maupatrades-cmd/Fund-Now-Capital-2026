...
export function DealPackageReadiness({ clientId, isPurchaseOrder, amountRequested }: { clientId: string | null; isPurchaseOrder: boolean; amountRequested: string | null }) {
  ...
  return <section className="rounded-xl border border-border bg-white p-5 shadow-sm">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><PackageCheck className="h-5 w-5 text-brand-teal" /><h2 className="font-semibold text-brand-navy">Deal package readiness</h2></div><p className="mt-1 text-xs text-muted-foreground">Baseline checks before sending a funding pack. Individual funders may require more.</p></div><span className={`rounded-full px-3 py-1 text-sm font-bold ${percentage===100?"bg-green-100 text-green-700":"bg-amber-100 text-amber-800"}`}>{percentage}% ready</span></div>
    <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-brand-teal" style={{ width: `${percentage}%` }} /></div>
    {query.isLoading ? <p className="mt-4 text-sm text-muted-foreground">Checking documents...</p> : <div className="mt-4 grid gap-2 sm:grid-cols-2">{checks.map((item) => <div key={item.label} className="flex items-center gap-2 text-sm">{item.ready?<CheckCircle2 className="h-4 w-4 text-green-600"/>:<Circle className="h-4 w-4 text-amber-500"/>}<span className={item.ready?"text-brand-navy":"text-muted-foreground"}>{item.label}</span></div>)}</div>}
    {clientId && percentage < 100 && <Link to={`/clients/${clientId}`} className="mt-4 inline-block text-sm font-semibold text-brand-teal hover:underline">Complete missing client documents →</Link>}
  </section>;
}
