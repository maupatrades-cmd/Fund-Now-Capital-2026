...
export function GlobalSearch() {
  ...
  return <div ref={root} className="relative hidden w-full max-w-sm md:block">
    ...
    {open && term.trim().length >= 2 && <div className="absolute left-0 right-0 top-11 z-30 max-h-80 overflow-y-auto rounded-xl border border-border bg-white p-2 shadow-xl">
      {query.isLoading && <div className="flex items-center gap-2 p-3 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Searching...</div>}
      {query.isError && <p className="p-3 text-sm text-red-600">Search is temporarily unavailable.</p>}
      {!query.isLoading && !query.isError && (query.data?.length ?? 0) === 0 && <p className="p-3 text-sm text-muted-foreground">No matches found.</p>}
      {(query.data ?? []).map((result) => <Link key={`${result.kind}-${result.id}`} to={result.to} onClick={() => { setOpen(false); setTerm(""); }} className="flex items-center justify-between rounded-lg px-3 py-2 text-sm hover:bg-slate-50"><span className="font-medium text-brand-navy">{result.label}</span><span className="text-xs text-muted-foreground">{result.kind}</span></Link>)}
    </div>}
  </div>;
}
