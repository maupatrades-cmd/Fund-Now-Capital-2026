...
/*
 * Tiny, dependency-free markdown renderer for training module content.
 *
 * Deliberately lane-local (not shared with the T&C renderer) so the TRAINING
 * lane stays self-contained and merges independently of the terms lane. Same
 * rationale as the codebase's other purpose-built renderers: the owner authors
 * training content in a small, known markdown subset, so pulling in react-
 * markdown + its transitive deps isn't warranted. XSS-safe by construction — we
 * only ever emit React text nodes and a fixed set of elements; raw HTML in the
 * source renders as literal text, never injected.
 *
 * Supported: `#`/`##`/`###` headings, `- ` bullet lists, blank-line-separated
 * paragraphs, `**bold**`, `_italic_`. Anything else renders as plain text.
 */

// Split a line into **bold** / _italic_ / plain-text spans.
...
export function TrainingMarkdown({ markdown }: { markdown: string }) {
