...
import { BadgeCheck, Bell, CalendarDays, ClipboardList, FileSignature, Files, Gauge, LogOut, MessagesSquare, Milestone, UserRound } from "lucide-react";
            ...
            {navigation.map((item) => (
              <li key={item.to}>
                <NavLink
                  ...
                  className={({ isActive }) => `flex items-center gap-3 rounded-2xl border px-4 py-3 text-sm font-semibold transition ${
                    isActive
          ...
          {mobileNavigation.map((item) => (
            <li key={item.to}>
              <NavLink
                ...
                className={({ isActive }) => `flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl text-[10px] font-bold ${
