...
- ✅ B2. Lead Entry + Qualification (Part 2): `leads` table, owner manual-entry form (full field set), qualification workflow (4 stages, 15 standard reasons), auto-create deal on "Qualified". Loaded-on-behalf fields (Part 4) included now. Fires LEAD_CREATED_FOR_YOU notification (owner-entered for Doctor). — B2.1 (lead entry) + B2.2 (`qualify_lead` workflow) + B2.3 (CIPC hard-block + advisory duplicate detection at entry, activity logging on lead events, lead-lifecycle notifications with email) all DONE, applied + verified live. B2.3 smoke test passed end-to-end (dup block/warn, activity + QUALIFICATION events, owner+partner notifications, partner-PII gates, CIPC-race unique index, server-authority raw-insert block); live test data cleaned up.
