...
- ⬜ A10. Activity Logging foundation (Part 4): `activity_logs` table, async triggers on CREATE/UPDATE for deals/leads/clients/commissions, per-entity Activity tab, owner timeline view. (Generalises the stage-history table from PR #9.)
- ⬜ A11. In-app Notification system (Part 4): `notifications` + `notification_deliveries` + `notification_preferences` tables, bell icon + dropdown + mark-as-read, Realtime badge updates
- ⬜ A12. Email notifications via Resend (Part 4, high-priority events only: LEAD_CREATED_FOR_YOU, DEAL_APPROVED, DEAL_FUNDED, COMMISSION_PAID) — includes Resend + Cloudflare DNS setup from Part 1 Phase 2
