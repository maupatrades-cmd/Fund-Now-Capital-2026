...
- 🔨 B5. Data validation pass (Part 6 M6, core slice): SA ID Luhn check, CIPC format, cell format, duplicate detection on client/lead creation. — **B5.1 ✅ DONE (PR #53), applied + verified live**: canonical `src/lib/sa-validation.ts` (Luhn SA ID + DOB/citizenship, CIPC, cell, VAT, postal), consolidated the drifted lead/client validators, wired Luhn into the lead + contact forms, added CIPC validation to the client form. **B5.2 (pending)**: `find_client_duplicates` RPC + server-side `clients` CIPC guard + client-form dup UX (a qualify×CIPC design fork is flagged for the owner's decision before build).
- ⬜ B6. Stakeholders — Directors, Shareholders, Beneficial Owners (Part 2): `client_stakeholders` (one row per person, `roles` array; SA ID **or** passport with ISO country; shareholding % + FICA beneficial-owner ≥25% auto-suggest), lead→client migration at qualify (documents pattern), owner-full / partner-PII-excluded RLS, activity logging. Opens **before Phase B closes**. Full spec + owner-locked decisions in **SPEC S3a**. Split: **B6.1** schema/RLS, **B6.2** UI.

_Phase B closes with B6. **Phase C opens next with C0** (per-funder commission structures — prerequisite for C1/C2)._
...
- **B4.1 — Client Story writable on lead detail (pre-qualification)** + export as structured markdown. See **SPEC S3**. 1 PR, ~2 hours. **Elevated to priority follow-up after B5** (not a "someday" item) — it is the substrate for the E3/E6 AI-assisted deal packaging suite (S6a auto-generation + S17b story refiner + S18 document intelligence). Suggested build order: **B5 → B4.1 → C0 → C1 …**
