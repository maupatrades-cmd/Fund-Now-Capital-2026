...
_**Phase B is COMPLETE** (B1–B6 + the B4.1 Client-Story-on-lead refinement, PR #63 — see Deferred Polish). **Phase C is now CURRENT, opening with C0** (per-funder commission structures — prerequisite for C1/C2). Phase B data-hygiene cleanup is **done** (test-leftover clients/deals/leads deleted 2026-07-20); the only remaining hygiene is 7 orphaned Storage blobs (owner deletes via dashboard) + the owner-reviewed Mama Mabase CIPC correction._

## PHASE C — Money Operations (Part 5) — CURRENT
...
- ⬜ **C0. Money Operations Foundations (opens Phase C — prerequisite for C1 and C2).** Per-funder commission structure tracking. Each funder in the CRM has its own commission rate that FNC earns from them per signed broker agreement. The commission engine today computes only the FNC-internal 40/60 split + tiered Doctor's share — it doesn't know what FNC earns from each specific funder. This module adds:
  ...
  **C0 must ship before C1 (invoicing) or C2 (commission auto-write)** — both depend on knowing what FNC earns per deal per funder. Owner intent: _"Each funder has its own rate structure per signed broker agreement. The CRM must know these rates to invoice correctly, compute honest commissions, and show accurate per-approval potential earnings on Doctor's Phase D portal."_ Full lifecycle spec in **SPEC S7A**.
  - **Parked (MDR mechanics deferred, owner decision 2026-07-20):** C0.1 ships the `percent_of_mdr` `rate_type` enum value, but the MDR **capture column + calculation are deferred** — none of the 21 current funders are MCA/card-settlement. **When the first MCA-funded deal opens (Phase C2), add `deal_funder_submissions.mdr_amount` (numeric(14,2))** as the per-deal captured MDR base, and finalise the `percent_of_mdr` math then.
