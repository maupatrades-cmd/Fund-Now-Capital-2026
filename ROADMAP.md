...
- ✅ B5. Data validation pass (Part 6 M6, core slice): SA ID Luhn check, CIPC format, cell format, duplicate detection on client/lead creation. — **B5.1 ✅ DONE (PR #53), applied + verified live**: canonical `src/lib/sa-validation.ts` (Luhn SA ID + DOB/citizenship, CIPC, cell, VAT, postal), consolidated the drifted lead/client validators, wired Luhn into the lead + contact forms, added CIPC validation to the client form. **B5.2 ✅ DONE (PRs #61/#62), applied + verified live**: `find_client_duplicates` RPC + server-side `clients_cipc_block` BEFORE-trigger + clients CIPC unique index + client-form dup UX. **B5 COMPLETE.**
- ✅ B6. Stakeholders — Directors, Shareholders, Beneficial Owners (Part 2): `client_stakeholders` (one row per person, `roles` array; SA ID **or** passport with ISO country; shareholding % + FICA beneficial-owner ≥25% auto-suggest), lead→client migration at qualify (documents pattern), owner-full / partner-PII-excluded RLS (`partner_stakeholders_view`), activity logging. Full spec + owner-locked decisions in **SPEC S3a**. — **✅ DONE (PRs #57 docs / #58 B6.1 schema / #59 B6.2 UI / #60 B6.3 qualify re-pointer), applied + verified live.** 0 rows by design (no backfill — owner enters real cap-table data).

_**PHASE B COMPLETE** (B4.1 story-on-lead also shipped, PR #63 window — see Deferred Polish). Phase C opened with C0._
...
- ✅ **C0. Money Operations Foundations — DONE (PRs #65 C0.1 schema / #66 C0.2 `/settings/funders` UI / #68 C0.3 contracted-gate + `percent_of_finance_charge` / #70 short_code index), applied + verified live.** Live: 11 contracted funders, 4 rate rows (all contracted, post PR #93 cleanup); `funder_rate_at()` effective-dated lookup. ⚠️ Better Banc + Business Partners still need `short_code` (owner data entry). Original scope (for reference): Per-funder commission structure tracking. Each funder in the CRM has its own commission rate that FNC earns from them per signed broker agreement. The commission engine today computes only the FNC-internal 40/60 split + tiered Doctor's share — it doesn't know what FNC earns from each specific funder. This module adds:
...
- ✅ C1. Invoicing FNC → funders — **DONE (C1.1 backend PR #72 + logo hotfixes #74–#76; C1.2 UI PR #78), applied + deployed.** As-built: single `funder_invoices` table (supersedes the invoices/line_items/payments outline — SPEC S7), lifecycle RPCs (generate/issue/mark-paid/void, advisory-locked + state-guarded), `generate-invoice-pdf` Edge Function + private `invoices` bucket, `check_overdue_invoices()` pg_cron 04:30 UTC, `/invoices` route + deal-detail Generate button. Sequence from INV-0032; 0 invoices issued yet (no genuine funding).
- ✅ C2. Commission records wiring — **DONE (C2.1 `deals.deal_type` cutover PR #79 · C2.2 reshape + `write_commission_record` + funded triggers PRs #80/#82 · C2.3 invoice-driven state cascade PR #83 · C2.4 `bonus_records` + BONUS_PAID PR #86), all applied + verified live.** `commission_records`/`bonus_records` legitimately 0 rows until the first genuine funding event; settle/unsettle stubs wire up at C4.
- 🔨 C3. Doctor's earnings engine: `doctor_earnings` lifecycle (earned → ready_to_invoice → invoiced → paid) — **owner-side `/partner-earnings` reconciliation view shipped (PR #91)**; the S8 lifecycle tables + partner-facing surface remain to build (scope proposal first; S7C presentation policy governs).
...
- ✅ **B4.1 — Client Story writable on lead detail (pre-qualification)** + export as structured markdown. See **SPEC S3**. — **DONE (PR #63 window), applied + verified live**: `client_stories` client_id XOR lead_id, qualify-time story+notes re-pointer (`STORY_MIGRATED_LEAD_TO_CLIENT`), `StoryPanel` on lead detail, markdown export. The E3/E6 substrate is in place.
...
## OPEN DECISIONS
1. ~~Doctor's Commission Estimator "Business View": full ranged internal-split breakdown (Part 3 spec) vs Doctor's-own-number-only. Blocks D5.~~ **RESOLVED (owner, 2026-07-21): his-number-only, presented as a display-only 50/50 split — SPEC S11.1 + S7C/S7C.1. D5 unblocked.**
...
3. Commission-engine SPEC reconciliation (before first affected deal; separate scope proposal — no engine change without owner sign-off): (a) flat-40% PO tier: all-PO in code vs Sourcefin-only in SPEC S7C; (b) R1M+ band: 25% in code vs TBD in S7C.
