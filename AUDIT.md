...
| **F6** | **Two contracted funders lack `short_code`** | `Business Partners` and `Better Banc` have `is_contracted = true`, `short_code = NULL`. `generate_funder_invoice` → `funder_shortcode()` needs the code for `payment_reference_expected`; an invoice generation against these funders will fail (or mint a malformed reference — behaviour to verify at fix time). Data entry by owner (they know the right codes), not a schema fix. Already flagged in status.md NEW CC 1 lane; still open. | Medium (blocks invoicing those two funders only) |
...
4. **Owner actions (no PR):** enable leaked-password protection (F3); enter `short_code` for Business Partners + Better Banc (F6); optionally delete the two inert Edge Functions; rule on D3 (PO scope + R1M+ tier) and the DEAL-001 stage question.
